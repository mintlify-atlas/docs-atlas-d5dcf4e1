...
Apply the same pattern for MongoDB (port 27017, target label `app: inventario-db`) and OpenLDAP (port 389, target label `app: ldap-service`). All three policy manifests live in `k8s/network-policies/` and are applied automatically when you run `kubectl apply -f k8s/`.
