...
<Info>
  When you add Kubernetes manifests for the MongoDB deployment, place them under `k8s/mongodb/`. Include a `PersistentVolumeClaim` for data durability and a `Service` definition that exposes port 27017 cluster-internally. See [Kubernetes deployment](/deployment/kubernetes) for guidance on the overall manifest structure.
</Info>
