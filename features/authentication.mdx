---
title: "Authenticate with LDAP and Active Directory"
sidebarTitle: "Authentication"
description: "InventarioITU authenticates all users against OpenLDAP or Active Directory. Learn how credentials flow from the web app through the LDAP service on port 389."
---
...
<Steps>
  <Step title="User submits the login form">
    The user opens `inventario-web` in a browser (port 3000) and enters their ITU username and password. The form POSTs the credentials over HTTPS to the Express `/login` route.
  </Step>
  ...
  <Step title="Session is created or the user is redirected">
    On a successful bind, the Express backend creates a server-side session, stores the user's DN and group membership, and redirects to the inventory dashboard. On failure, the user is redirected back to `/login` with an error message — no session is written and no internal details are exposed to the browser.
  </Step>
</Steps>
...
<CardGroup cols={2}>
  <Card title="Admin" icon="shield-check">
    Members of the `inventario-admins` LDAP group. Can create, edit, and delete equipment records in both SQL Server and MongoDB, manage user assignments, and access all reports.
  </Card>
...
```yaml
# k8s/frontend/deployment.yaml (excerpt)
env:
  - name: LDAP_URL
