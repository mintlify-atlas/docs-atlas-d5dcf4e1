...
description: "Set up OpenLDAP as the authentication backend for InventarioITU. Configure the directory tree, add user accounts, and connect the Node.js app via environment variables."
---
...
<Note>
The `ldap/` directory in the InventarioITU repository contains configuration templates (LDIF files and schema definitions) that you can use as a starting point. Apply them with `ldapadd` after the container is running.
</Note>
